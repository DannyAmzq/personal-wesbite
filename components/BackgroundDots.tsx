...
export default function BackgroundDots({ count = 40 }: { count?: number }) {
  ...
  useEffect(() => {
    ...
    const handle = (e: PointerEvent) => {
      const x = e.clientX / (viewport.w || 1) - 0.5;
      const y = e.clientY / (viewport.h || 1) - 0.5;
      // Subtle parallax: max 12px offset
      mx.set(x * 12);
      my.set(y * 12);
    };
    window.addEventListener("pointermove", handle);
    return () => window.removeEventListener("pointermove", handle);
  }, [mx, my, viewport.w, viewport.h, reduce]);
